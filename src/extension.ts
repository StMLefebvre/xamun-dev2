...
export function activate(context: vscode.ExtensionContext) {
	...
	context.subscriptions.push(
		vscode.window.registerWebviewViewProvider(XamunProvider.sideBarId, sidebarProvider, {
			webviewOptions: { retainContextWhenHidden: true },
		})
	)

	context.subscriptions.push(
		vscode.commands.registerCommand("xamun-dev.plusButtonClicked", async () => {
	...
	const openClineInNewTab = async () => {
		...
		const tabProvider = new XamunProvider(context, outputChannel)
		...
		const panel = vscode.window.createWebviewPanel(XamunProvider.tabPanelId, "Cline", targetCol, {
			enableScripts: true,
	...
	context.subscriptions.push(vscode.commands.registerCommand("xamun-dev.popoutButtonClicked", openClineInNewTab))
	context.subscriptions.push(vscode.commands.registerCommand("xamun-dev.openInNewTab", openClineInNewTab))

	context.subscriptions.push(
		vscode.commands.registerCommand("xamun-dev.settingsButtonClicked", () => {
	...
	context.subscriptions.push(
		vscode.commands.registerCommand("xamun-dev.historyButtonClicked", () => {
	...
	*/
	...
	const handleUri = async (uri: vscode.Uri) => {
		...
		const visibleProvider = XamunProvider.getVisibleInstance()
		if (!visibleProvider) {
			return
		}
		switch (path) {
